...
**_注意_**如果同时指定了'day of month'和'day of week'为一个严格的值（即不是星号），则在任何一个条件满足的情况下，计划任务都会被执行。如："30 4 1,15 * 5"将会在每个月的1号和15号以及每周星期五的上午4:30执行。如果有一个指定了严格的值，而另一个指定为星号，则星号将被忽略。
...
- "0 0 1 1 *"为什么是@yearly？
    ...
    **_解答_**：当同时指定了'day of month'和'day of week'为一个严格的值（即不是星号），则在任何一个条件满足的情况下，计划任务都会被执行。这里有一个前提条件是他们的值都是严格值，即非星号，所以上面'day of week'的值为星号，自然被忽略了。

### 注意事项 <a id='CT-NOTE'>[≡](#≡)</a>

- 定时任务脚本第一行注释中包含中文时无法运行。

    该注意事项摘自网络[crontab环境变量](https://my.oschina.net/dclink/blog/194941)，文章中提到，当定时任务的脚本第一行为注释内容，且注释内容中带有中文时，可能会导致脚本运行失败！最好是在第一行指定sha-bang（即#!/bin/sh）
